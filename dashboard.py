...
df_ventas = pd.read_csv('https://raw.githubusercontent.com/dlssergio/sales_dashboard_streamlit/main/base_ventas.csv')
...
if not anios:
    anio = st.sidebar.slider('Año', df_ventas['order_purchase_timestamp'].dt.year.min(), df_ventas['order_purchase_timestamp'].dt.year.max())

# Filtrando los datos (Agregando interacción)
if ciudades:
...
if not anios:
    df_ventas = df_ventas[df_ventas['order_purchase_timestamp'].dt.year == anio]
